...
public class SpeechBlend : MonoBehaviour
{
    ...
    [HideInInspector] public bool showBlendShapeMenu = false;
    [HideInInspector] public SpeechUtil.VisemeBlendshapeIndexes faceBlendshapes;
    [HideInInspector] public SpeechUtil.VisemeWeight visemeWeightTuning;

    [Header("Settings")] [Space(10)] [Tooltip("Toggle lipsyncing")]
...
#endif
    [Tooltip("Amplitude of jaw movement")] [Range(0.0f, 1.0f)]
    public float jawMovementAmount = 0.5f; // Amplitude of jaw movement

    [Tooltip("Jaw motion speed")] [Range(0.0f, 1.0f)]
    public float jawMovementSpeed = 0.5f; // Jaw motion speed

    [Tooltip("Amplitude of lip movement")] [Range(0.0f, 1.0f)]
    public float lipsBlendshapeMovementAmount = 0.5f; // Amplitude of lip movement

    [Tooltip("Lip viseme movement speed")] [Range(0.0f, 1.0f)]
    public float lipsBlendshapeChangeSpeed = 0.5f; // Lip viseme movement speed

    [Tooltip("Number of calculations to use.")]
    public SpeechUtil.Accuracy
        accuracy = SpeechUtil.Accuracy.Medium; // Number of calculations to use. Cannot be changed at runtime 

    [Tooltip("Number of frames to wait before next calculation (higher number uses less resources but reponds slower)")]
    [Range(1, 10)]
    public int
        framesPerUpdate =
            1; // Number of frames to wait before next calculation (higher number uses less resources but reponds slower)   
    ...
    public VoiceProfile.VisemeBlendshapeTemplate
        shapeTemplate; // Blendshape template for visemes shapes. (default: DAZ)
    ...
    [HideInInspector] public SpeechUtil.VisemeBlendshapeIndexes faceBlendshapes_saved =
        new SpeechUtil.VisemeBlendshapeIndexes(VoiceProfile.G2_template);
    ...
    void Start()
    {
        ...
        voiceAudioSource = transform.parent.GetComponentInChildren<AudioSource>();
    }

    private Coroutine lipAnimator;

    public void PlaySound(AudioClip sound, AudioSource source)
    {
        if (source == null)
        {
            //Debug.LogWarning("No audio source for this text!");
            return;
        }

        if (sound == null)
        {
            //Debug.LogWarning("No audio clip for this text!");
            return;
        }

        voiceAudioSource = source;
        var len = sound.length;
        voiceAudioSource.clip = sound;
        voiceAudioSource.playOnAwake = false;
        voiceAudioSource.loop = false;
        voiceAudioSource.mute = false;
        voiceAudioSource.Play();
        if (lipAnimator != null) StopCoroutine(lipAnimator);
        if (gameObject.activeInHierarchy)
            lipAnimator = StartCoroutine(CR_LipAnimator(len));
    }
    ...
    IEnumerator CR_LipAnimator(float len)
    {
        lipsyncActive = true;
    ...
    void FixedUpdate()
    {
        ...
        if (voiceAudioSource.isPlaying & lipsyncActive)
        {
            ...
            if (++updateFrame >= framesPerUpdate)
            {
                ...
                current_volume = last_volume * (1 - lipsBlendshapeChangeSpeed) +
                                 current_volume * lipsBlendshapeChangeSpeed;
                ...
                {
                    //current_volume = 0;
                    trans_mouthOpen_setpoint = Quaternion.Euler(jawJointOffset +
                                                                trans_mouthOpen_rest.eulerAngles *
                                                                (1 - jawMovementAmount * 3) +
                                                                (trans_mouthOpen_rest.eulerAngles +
                                                                 jawOpenDirection * current_volume /
                                                                 jaw_volume_scaling) * jawMovementAmount * 3);
                }
...
#endif
                    float[] features = ExtractFeatures.ExtractSample(rawData, extractor, transformer, modifier,
                        ref cmem, f_low, f_high, accuracy);
    ...
    private void LateUpdate()
    {
        ...
        if (!faceBlendshapes.BlendshapeAssigned("mouthOpen"))
        {
            if (jawJoint != null)
            {
                ...
                jawJoint.transform.localRotation = Quaternion.Lerp(jawJoint.transform.localRotation,
                    trans_mouthOpen_setpoint, Time.deltaTime * moveSpeed);
            }
        }
    ...
    void UpdateBlendshapes()
    {
        if (trackingMode == SpeechUtil.Mode.jawAndVisemes)
        {
            for (int i = 0; i < faceBlendshapes.template.Nvis; i++)
            {
                if (faceBlendshapes.BlendshapeAssigned(i) & blendshapeInfluenceActive[i])
                {
                    ...
                    var temp = faceBlendshapes.GetByIndex(i);
                    if (lipsyncActive)
                    {
                        headMesh.SetBlendShapeWeight(temp,
                            currentVisemeValue * (1 - lipsBlendshapeChangeSpeed * bs_CSF) +
                            bs_setpoint[i] * lipsBlendshapeChangeSpeed * bs_CSF);
        ...
        if (faceBlendshapes.BlendshapeAssigned("mouthOpen"))
        {
            if (lipsyncActive)
            {
                ...
                headMesh.SetBlendShapeWeight(faceBlendshapes.mouthOpenIndex,
                    currentValue * (1 - (jawMovementSpeed * jaw_CSF)) +
                    bs_mouthOpen_setpoint * (jawMovementSpeed * jaw_CSF));
            }
        }
    }
    ...
    {
        extractor = ExtractFeatures.BuildExtractor(fres, ExtractFeatures.getlf(accuracy),
            ExtractFeatures.gethf(accuracy), accuracy);
