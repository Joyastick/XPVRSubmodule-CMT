...
namespace XNodeEditor
{
    public partial class NodeEditorWindow
    {
        ...
        public XNode.NodePort DraggedOutputPort
        {
            get
            {
                XNode.NodePort result = draggedOutput;
                return result;
            }
        }
        ...
        public XNode.NodePort HoveredPort
        {
            get
            {
                XNode.NodePort result = hoveredPort;
                return result;
            }
        }
            ...
            switch (e.type)
            {
                ...
                case EventType.MouseUp:
                    if (e.button == 0)
                    {
                        ...
                        {
                            IEnumerable<XNode.Node> nodes = Selection.objects.Where(x => x is XNode.Node)
                                .Select(x => x as XNode.Node);
        ...
        {
            copyBuffer = Selection.objects.Select(x => x as XNode.Node).Where(x => x != null && x.graph == graph)
                .ToArray();
        }
